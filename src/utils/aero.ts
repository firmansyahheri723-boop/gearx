import type { AeroSettings, AeroStandardOutput, AeroExperimentalOutput, AeroSpeedDataPoint, CarData } from '../types';
...
export function calculateStandardAero(settings: AeroSettings): AeroStandardOutput {
  ...
  if (balancePct > 60) {
    behavior = 'understeer';
    recommendations.push('Consider reducing front aero if experiencing high-speed understeer');
  ...
  if (settings.airResistance > 5) {
    recommendations.push('Consider reducing air resistance for higher top speed');
  }
...
export function calculateExperimentalAero(
  ...
  const frontDownforceN = 0.5 * AIR_DENSITY * frontalArea * frontCl * velocitySquared;
  const rearDownforceN = 0.5 * AIR_DENSITY * frontalArea * rearCl * velocitySquared;
  const totalDownforceN = frontDownforceN + rearDownforceN;

  const cd = (10 - settings.airResistance) * GAME_CD_FACTOR * dragMultiplier + dragCoefficient * 0.5;
  const dragForceN = 0.5 * AIR_DENSITY * frontalArea * cd * velocitySquared;
  const powerLossKw = (dragForceN * velocityMs) / 1000;

  const topSpeedReductionKmh = enginePowerKw > 0 
    ? Math.min((powerLossKw / enginePowerKw) * 50, 50)
    : 0;
  ...
  const speedData = generateSpeedData(settings, carData, frontalArea, dragCoefficient, dragMultiplier);

  const maxSpeedKmh = calculateMaxSpeedKmh(settings, carData, frontalArea, dragCoefficient, dragMultiplier, enginePowerKw);
...
function generateSpeedData(
  ...
  for (let speedKmh = 0; speedKmh <= MAX_SPEED_PLOT_KMH; speedKmh += SPEED_STEP) {
...
function calculateMaxSpeedKmh(
  ...
  while (high - low > 1) {
    ...
    const dragKw = (0.5 * AIR_DENSITY * frontalArea * cd * dragMultiplierFactor * velocityMs * velocityMs * velocityMs) / 1000;
