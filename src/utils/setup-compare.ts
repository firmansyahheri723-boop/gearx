...
function determineCategory(path: string): 'general' | 'suspension' | 'gearbox' | 'aero' | 'alignment' {
  const pathLower = path.toLowerCase();

  if (pathLower.includes('alignment') || pathLower.includes('camber') || pathLower.includes('caster') ||
      pathLower.includes('toe') || pathLower.includes('ackermann') || pathLower.includes('steering')) {
    return 'alignment';
  }

  if (pathLower.includes('aero') || pathLower.includes('drag') || pathLower.includes('downforce')) {
    return 'aero';
  }

  if (pathLower.includes('gear') || pathLower.includes('ratio') || pathLower.includes('finaldrive') ||
      pathLower.includes('finalDrive') || pathLower.includes('torque') || pathLower.includes('traction')) {
    return 'gearbox';
  }

  if (pathLower.includes('spring') || pathLower.includes('damper') || pathLower.includes('roll') ||
      pathLower.includes('frequency') || pathLower.includes('stiffness') || pathLower.includes('arb') ||
      pathLower.includes('weight') || pathLower.includes('cog') || pathLower.includes('wheel')) {
    return 'suspension';
  }
...
function getFieldLabel(path: string): string {
  ...
  return labels[path] ?? path.replace(/\./g, ' ').replace(/([A-Z])/g, ' $1').replace(/^./, (s) => s.toUpperCase()).trim();
}
...
function compareObjects(
  ...
  allKeys.forEach((key) => {
    ...
    if (typeof valueA === 'object' && typeof valueB === 'object' &&
      ...
      diffs.push(...compareObjects(valueA as Record<string, unknown>, valueB as Record<string, unknown>, path));
    } else {
      diffs.push({
        path,
...
function compareArrays(arrA: unknown[], arrB: unknown[], path: string): SetupDiffField[] {
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    if (typeof valA === 'object' && typeof valB === 'object' &&
      ...
      diffs.push(...compareObjects(valA as Record<string, unknown>, valB as Record<string, unknown>, `${path}[${i}]`));
    } else {
      const gearLabel = path.includes('gearRatios') ? `Gear ${i + 1}` : `Item ${i}`;
      diffs.push({
        path: `${path}[${i}]`,
        label: `${gearLabel} ${path.includes('gearRatios') ? 'Ratio' : ''}`.trim(),
        category: determineCategory(path),
        impact: hasDiff ? (path.includes('gearRatios') ? 'medium' : determineImpact(path, valA, valB)) : 'low',
        hasDifference: hasDiff,
        oldValue: valA,
        newValue: valB,
