...
export function calculateAlignmentOutputs(
	...
	const frontCamberGain = Math.abs(
		maxSteeringAngle * Math.sin((frontCamber * Math.PI) / 180),
	);
	const rearCamberGain = Math.abs(
		maxSteeringAngle * Math.sin((rearCamber * Math.PI) / 180),
	);
	...
	const understeerTendency = Math.min(
		100,
		Math.max(-100, (frontGripFactor - rearGripFactor) * 10 + frontToe * 5),
	);
	...
	const straightLineStability = Math.max(
		0,
		Math.min(
			100,
...
function generateRecommendations(
	inputs: AlignmentInputs,
	outputs: Partial<AlignmentOutputs>,
): string[] {
	const recs: string[] = [];
	...
	if (outputs.ackermannType === "reverse") {
		recs.push(
			"Reverse Ackermann is rare - consider parallel or positive for easier control",
		);
	}
