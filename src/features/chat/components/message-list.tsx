...
export function MessageList(props: MessageListProps) {
  ...
  return (
    ...
    >
          ...
          {(message) => (
            ...
            >
              <div
                ...
                classList={{
                  'bg-foreground/10 text-foreground whitespace-pre-wrap': message.role === 'user',
              ...
              >
                ...
                >
                  <SolidMarkdown remarkPlugins={[remarkGfm]}>{message.content}</SolidMarkdown>
                </Show>
                <Show when={message.role === 'assistant' && props.isLoading && message === props.messages[props.messages.length - 1]}>
      ...
      <Show when={props.isLoading && (props.messages.length === 0 || props.messages[props.messages.length - 1]?.role !== 'assistant')}>
        <div class="flex justify-start">
          <div class="bg-surface-elevated/50 text-foreground-secondary px-3 py-2 text-sm">
            <span class="inline-flex gap-1">
              <span class="w-2 h-2 bg-foreground/50 rounded-full animate-bounce" style={{ "animation-delay": "0ms" }} />
              <span class="w-2 h-2 bg-foreground/50 rounded-full animate-bounce" style={{ "animation-delay": "150ms" }} />
              <span class="w-2 h-2 bg-foreground/50 rounded-full animate-bounce" style={{ "animation-delay": "300ms" }} />
            </span>
          </div>
        </div>
      </Show>
