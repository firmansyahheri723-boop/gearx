import type {
	ExtractedDataPoint,
	TorqueExtractorCalibration,
	TorqueExtractorPoint,
} from "./types";
...
export function extractCyanPixels(
	imageData: ImageData,
...
export function clusterAndCleanPixels(
	pixels: TorqueExtractorPoint[],
): { x: number; medianY: number }[] {
...
export function samplePoints(
	cleanPixels: { x: number; medianY: number }[],
	numSamples: number,
): TorqueExtractorPoint[] {
...
export function pixelToValues(
	pixel: TorqueExtractorPoint,
	calibration: TorqueExtractorCalibration,
): { rpm: number; torque: number } {
	const { left, right, top, bottom, minRpm, maxRpm, minTorque, maxTorque } =
		calibration;
...
export function extractTorqueCurve(
	imageData: ImageData,
	calibration: TorqueExtractorCalibration,
): ExtractedDataPoint[] {
