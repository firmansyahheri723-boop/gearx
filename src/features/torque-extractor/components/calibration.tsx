...
export function CalibrationOverlay(props: CalibrationOverlayProps) {
	...
	const handlePointerDown = (e: PointerEvent) => {
		...
		if (
			getLineDistance(x, y, { x1: xMin, y1: c.top, x2: xMax, y2: c.top }) <
			threshold
		) {
			target = "top";
		} else if (
			...
			getLineDistance(x, y, { x1: c.left, y1: yMin, x2: c.left, y2: yMax }) <
			threshold
		) {
			target = "left";
		} else if (
			getLineDistance(x, y, { x1: c.right, y1: yMin, x2: c.right, y2: yMax }) <
			threshold
		) {
			target = "right";
		}
		...
		<div class="flex flex-col gap-4">
			<div class="flex items-center justify-between">
				...
				<button
					...
					class="border px-4 py-2 text-xs uppercase tracking-wider transition-colors cursor-pointer"
				>
					Extract Curve
				</button>
			</div>
			...
			<div class="grid grid-cols-2 gap-4 p-3 border border-border bg-surface/50">
				<div class="space-y-2">
					<span class="text-[10px] text-muted uppercase tracking-wider block">
						RPM Scale
					</span>
					<div class="flex gap-2 items-center">
						...
						<NumberInput
							...
							onChange={(val) =>
								props.onChange({ ...props.calibration, minRpm: val })
							}
					...
					<div class="flex gap-2 items-center">
						...
						<NumberInput
							...
							onChange={(val) =>
								props.onChange({ ...props.calibration, maxRpm: val })
							}
							class="flex-1 bg-surface-elevated border border-border text-foreground px-2 py-1.5 text-xs focus:border-border focus:outline-none"
						/>
					</div>
				...
				<div class="space-y-2">
					...
					<div class="flex gap-2 items-center">
						...
						<NumberInput
							...
							onChange={(val) =>
								props.onChange({ ...props.calibration, minTorque: val })
							}
							class="flex-1 bg-surface-elevated border border-border text-foreground px-2 py-1.5 text-xs focus:border-border focus:outline-none"
						/>
					</div>
					<div class="flex gap-2 items-center">
						...
						<NumberInput
							...
							onChange={(val) =>
								props.onChange({ ...props.calibration, maxTorque: val })
							}
							class="flex-1 bg-surface-elevated border border-border text-foreground px-2 py-1.5 text-xs focus:border-border focus:outline-none"
						/>
					</div>
