...
> = {
	...
	dampers: {
		...
		articles: [
			{
				label: "Wikipedia: Shock Absorber",
				url: "https://en.wikipedia.org/wiki/Shock_absorber",
			},
		],
		videos: [
			{
	...
	accelerationMetrics: {
		...
		articles: [
			{
				label: "Wikipedia: Weight Transfer",
...
export function SuspensionOutput(props: SuspensionOutputProps) {
	...
	const damperData = (): DamperRow[] => [
		{
			label: "Bump",
			front: Math.round(props.outputs.dampers.bumpFront),
			rear: Math.round(props.outputs.dampers.bumpRear),
		},
		{
			label: "Fast bump",
			front: Math.round(props.outputs.dampers.fastBumpFront),
			rear: Math.round(props.outputs.dampers.fastBumpRear),
		},
		{
			label: "Rebound",
			front: Math.round(props.outputs.dampers.reboundFront),
			rear: Math.round(props.outputs.dampers.reboundRear),
		},
		{
			label: "Fast rebound",
			front: Math.round(props.outputs.dampers.fastReboundFront),
			rear: Math.round(props.outputs.dampers.fastReboundRear),
		},
	];
	...
	const metricsData = (): MetricRow[] => [
		{
			label: "Weight transfer on accel",
			value: `${props.outputs.acceleration.weightTransfer.toFixed(1)} kg`,
		},
		{
			label: "Front weight dist on accel",
			value: `${props.outputs.acceleration.frontBiasOnAccel.toFixed(1)}%`,
		},
		{
			label: "Max longitudinal accel",
			value: `${props.outputs.acceleration.longitudinalAccelG.toFixed(2)} g`,
		},
		{
			label: "Max lateral accel",
			value: `${props.outputs.acceleration.lateralAccelG.toFixed(2)} g`,
		},
	];
	...
	return (
		<div class="border border-border/50 bg-background/50">
			...
			<div class="border-b border-border/30">
				...
				<DataTable data={damperData()} columns={damperColumns} />
			</div>
			...
			<div class="border-t border-border/30">
				...
				<DataTable data={metricsData()} columns={metricColumns} />
			</div>
		</div>
	);
}
