		...
		>
			<table class="w-full border-collapse text-sm">
				<thead class={merged.stickyHeader ? "sticky top-0 z-20" : ""}>
					...
					<For
						each={
							merged.showHeaderGroups
								? table.getHeaderGroups().slice(-1)
						...
						{(headerGroup) => (
							<tr>
								<For each={headerGroup.headers}>
									{(header) => {
										const canSort = () =>
											merged.enableSorting && header.column.getCanSort();
										...
										return (
											<th
												...
												classList={{
													"cursor-pointer select-none hover:bg-surface-elevated/50":
														canSort(),
													"text-left":
				...
				<tbody>
					<For each={table.getRowModel().rows}>
						{(row, index) => {
							const rowClass = () =>
								merged.getRowClass?.(row.original, index()) ?? "";
